...
//   // dut.reset := (ldut.reset.asBool | ldut.debug.map { debug => AsyncResetReg(debug.ndreset) }.getOrElse(false.B)).asBool

//   val customReset = (this.reset.asBool || ldut.debug.map { debug =>
//     AsyncResetReg(debug.ndreset).asBool
//   }.getOrElse(false.B)).asAsyncReset
